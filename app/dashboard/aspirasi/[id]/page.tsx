      ...
      <div className="md:col-span-2 space-y-6">
        ...
        <div className="glass-panel p-8 rounded-xl border border-white/5">
          <div className="flex justify-between items-start mb-6 border-b border-white/5 pb-4">
            <div>
              <span className="px-3 py-1 rounded-full text-xs bg-slate-800 text-indigo-400 border border-slate-700 mb-3 inline-block font-medium">
            ...
            <div className="text-right">
              <p className="text-xs text-slate-500 uppercase tracking-wider font-semibold">ID Laporan</p>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div>
               ...
               <p className="text-slate-200 font-medium">{aspirasi.input_aspirasi.siswa.nama}</p>
            ...
            <div>
               ...
               <p className="text-slate-200 font-medium">{aspirasi.input_aspirasi.lokasi}</p>
            </div>
          </div>

          <div className="mb-6">
             <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Deskripsi Detail</h3>
             <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-800 text-slate-300 leading-relaxed whitespace-pre-wrap text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-800/30 p-4 rounded-lg border border-slate-700/50">
              ...
              <p className={`font-bold capitalize ${
                ...
                aspirasi.status_validasi === 'ditolak' ? 'text-red-400' : 'text-amber-400'
              }`}>
                {aspirasi.status_validasi === 'pending' ? 'Menunggu Admin' :
                 aspirasi.status_validasi === 'diajukan' ? 'Menunggu Yayasan' :
                 aspirasi.status_validasi}
              </p>
              {aspirasi.catatan_validasi && (
                 <p className="text-sm text-slate-400 mt-2 italic border-t border-slate-700/50 pt-2">"{aspirasi.catatan_validasi}"</p>
              )}
            </div>
            <div className="bg-slate-800/30 p-4 rounded-lg border border-slate-700/50">
      ...
      <div className="md:col-span-1 space-y-6">
        {(user.role === 'admin' || user.role === 'yayasan') && (
          <div className="glass-panel p-6 rounded-xl border border-white/5 sticky top-24">
            <h2 className="text-lg font-bold text-white mb-4 border-b border-white/5 pb-2">Panel Kontrol</h2>
            <ActionsForm 
              aspirasi={aspirasi} 
        ...
        {/* Feedback Section for Siswa */}
        {user.role === 'siswa' && aspirasi.status_progres === 'selesai' && !aspirasi.rating && (
          <div className="sticky top-24">
            <FeedbackForm aspirasiId={aspirasi.id} />
          </div>
        )}
        ...
        {aspirasi.rating && (
          <div className="glass-panel p-6 rounded-xl border border-white/5 sticky top-24">
            <h3 className="text-white font-bold mb-4 flex items-center gap-2 text-sm uppercase tracking-widest">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-amber-400" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
              Feedback Siswa
            </h3>
            <div className="flex gap-1 mb-3">
              {[1, 2, 3, 4, 5].map((star) => (
                <svg key={star} xmlns="http://www.w3.org/2000/svg" className={`w-5 h-5 ${(aspirasi.rating ?? 0) >= star ? 'text-amber-400' : 'text-slate-700'}`} viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
              ))}
            </div>
            {aspirasi.feedback ? (
              <p className="text-slate-300 text-sm italic bg-white/5 p-3 rounded-lg border border-white/5 leading-relaxed">
                "{aspirasi.feedback}"
              </p>
            ) : (
              <p className="text-slate-500 text-xs italic">Siswa hanya memberikan rating.</p>
            )}
          </div>
        )}
      </div>
